...
def parse_narc_content(content):
	...
	for table in soup.newdataset.find_all('table1'):
		try:
			record = {
				'병동' : table.ward.text if table.ward else "",
				'병  동' : table.ward_room_bed.text if table.ward_room_bed else "",
				'출력용 병실' : table.prt_room.text if table.prt_room else "",
				'cln_dept_cd' : table.cln_dept_cd.text if table.cln_dept_cd else "",
				'cln_dept_nm' : table.cln_dept_nm.text if table.cln_dept_nm else "",
				'원처방일자' : table.ori_ord_ymd.text if table.ori_ord_ymd else "",
				'처방시행일자' : table.ord_exec_ymd.text if table.ord_exec_ymd else "",
				'ord_no' : table.ord_no.text if table.ord_no else "",
				'환자번호' : table.ptnt_no.text if table.ptnt_no else "",
				'환자명' : table.ptnt_nm.text if table.ptnt_nm else "",
				'약품코드' : table.drug_cd.text if table.drug_cd else "",
				'약품명' : table.drug_nm.text if table.drug_nm else "",
				'약품명(정렬용)' : table.drug_nm_sort.text if table.drug_nm_sort else "",
				'chng_qty_unit' : table.chng_qty_unit.text if table.chng_qty_unit else "",
				'ord_qty' : table.ord_qty.text if table.ord_qty else "",
				'ord_unit_nm' : table.ord_unit_nm.text if table.ord_unit_nm else "",
				'처방량(규격단위)' : table.ord_qty_std.text if table.ord_qty_std else "",
				'규격단위' : table.std_unit_nm.text if table.std_unit_nm else "",
				'ord_qty_gb' : table.ord_qty_gb.text if table.ord_qty_gb else "",
				'횟수' : table.ord_frq.text if table.ord_frq else "",
				'일수' : table.ord_day.text if table.ord_day else "",
				'총량' : table.sum_qty.text if table.sum_qty else "",
				'집계량' : table.tot_qty.text if table.tot_qty else "",
				'용법코드' : table.dsg_cd.text if table.dsg_cd else "",
				'용법명' : table.dsg_nm.text if table.dsg_nm else "",
				'주상병명' : table.main_diag_nm.text if table.main_diag_nm else "",
				'불출생성일자' : table.ord_owarh_ymd.text if table.ord_owarh_ymd else "",
				'불출생성차수' : table.ord_owarh_dgr.text if table.ord_owarh_dgr else "",
				'접수일련번호' : table.rcpt_seq.text if table.rcpt_seq else "",
				'접수처방일련번호' : table.rcpt_ord_seq.text if table.rcpt_ord_seq else "",
				'접수생성일자' : table.rcpt_crtn_ymd.text if table.rcpt_crtn_ymd else "",
				'처방일자' : table.ord_ymd.text if table.ord_ymd else "",
				'연령' : table.age.text if table.age else "",
				'성별' : table.sex.text if table.sex else "",
				'sex_age' : table.sex_age.text if table.sex_age else "",
				'체중' : table.wght.text if table.wght else "",
				'주소' : table.addr.text if table.addr else "",
				'반납량(폐기량)' : table.funsh_qty.text if table.funsh_qty else "",
				'재고량' : table.stock_qty.text if table.stock_qty else "",
				'drgst_prog_stus' : table.drgst_prog_stus.text if table.drgst_prog_stus else "",
				'owarh_yn' : table.owarh_yn.text if table.owarh_yn else "",
				'불출자ID' : table.narct_owarh_id.text if table.narct_owarh_id else "",
				'불출자명' : table.narct_owarh_id_nm.text if table.narct_owarh_id_nm else "",
				'불출일자' : table.narct_owarh_ymd.text if table.narct_owarh_ymd else "",
				'narct_get_id_nm' : table.narct_get_id_nm.text if table.narct_get_id_nm else "",
				'narct_owarh_hms' : table.narct_owarh_hms.text if table.narct_owarh_hms else "",
				'narct_owarh_dt' : table.narct_owarh_dt.text if table.narct_owarh_dt else "",
				'iwarh_exist_yn' : table.iwarh_exist_yn.text if table.iwarh_exist_yn else "",
				'insu_tp_nm' : table.insu_tp_nm.text if table.insu_tp_nm else "",
				'issu_dept_cd' : table.issu_dept_cd.text if table.issu_dept_cd else "",
				'진료의' : table.cln_dr_id.text if table.cln_dr_id else "",
				'진료의명' : table.cln_dr_nm.text if table.cln_dr_nm else "",
				'처방의' : table.ord_dr_id.text if table.ord_dr_id else "",
				'처방의명' : table.ord_dr_nm.text if table.ord_dr_nm else "",
				'의사면허번호' : table.ord_dr_lcns_no.text if table.ord_dr_lcns_no else "",
				'우편번호' : table.zip.text if table.zip else "",
				'환자전화번호' : table.ptnt_tel_no.text if table.ptnt_tel_no else "",
				'접수약품구분명' : table.rcpt_drug_gb_nm.text if table.rcpt_drug_gb_nm else "",
				'수가코드' : table.suga_cd.text if table.suga_cd else "",
				'급여구분명' : table.kub_gb_nm.text if table.kub_gb_nm else "",
				'약국진행상태명' : table.drgst_prog_stus_nm.text if table.drgst_prog_stus_nm else "",
				'hosi_resn_desc' : table.hosi_resn_desc.text if table.hosi_resn_desc else "",
				'drug_rmk' : table.drug_rmk.text if table.drug_rmk else "",
				'반납여부' : table.ret_yn.text if table.ret_yn else "",
				'반납구분' : table.ret_gb.text if table.ret_gb else "",
				'마약접수구분' : table.narct_rcpt_gb.text if table.narct_rcpt_gb else "",
				'수령부서' : table.get_dept_nm.text if table.get_dept_nm else "",
				'envl_title' : table.envl_title.text if table.envl_title else "",
				'ord_qty_frq' : table.ord_qty_frq.text if table.ord_qty_frq else "",
				'ord_sum_gb' : table.ord_sum_gb.text if table.ord_sum_gb else "",
				'prt_ord_cap' : table.prt_ord_cap.text if table.prt_ord_cap else "",
				'prt_dsg_nm' : table.prt_dsg_nm.text if table.prt_dsg_nm else "",
			}
		except:
			continue
		else:
